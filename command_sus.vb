...
Module command_sus
    Async Sub run(message As IMessage, memberToTest As String)
        ...
        Dim img1 As String = "https://bot.shadowcat.club/api/sus/style.php?avatar=" & avatarEncoded & "&username=" & user.Username & "&discrim=" & user.Discriminator
        Dim vOut() As Byte = System.Text.Encoding.UTF8.GetBytes(img1)
        Dim b64encoded As String = System.Convert.ToBase64String(vOut)
        Dim r = New System.Net.WebClient().DownloadString("https://bot.shadowcat.club/api/sus/image.php?url=" & b64encoded)
        eb.WithImageUrl(r)
